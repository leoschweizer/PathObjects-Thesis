...
The structure of a \textsc{PathObjects} trace can be interpreted as directed multigraph $G=(V,E)$, meaning that the edges constitute a multiset rather than a set.
...
Contrarily to the traditional definition of simple directed graphs, loops have to be allowed, since message exchange between objects is bidirectional more than often.
...
Consequently, this allows to express the number of messages $n_M$ that are exchanged between two objects as cardinality of $M(N_1, N_2)$ (cf. Formula \ref{eq:ImplementationGraphMessageCardinality}).

\begin{equation}
n_M(N_1, N_2) = \left\vert \phantom{i} M(N_1, N_2) \phantom{i} \right\vert
...
\begin{equation}
e(N_1, N_2) = min(n_M(N_1, N_2),\phantom{i} \text{MAX\_MESSAGES})
\end{equation}
