...
In the previous chapter, we presented the fundamental components of \textsc{PathObjects} diagrams.
Section \ref{s:ImplementationPrototype} illustrates the realization of these components.
Section \ref{s:ImplementationTracing} describes the necessary extensions of the step-wise run-time analysis approach that allow us to reconstruct object interactions and referential relationships.
...
In order to allow for the reconstruction of object interactions from execution traces, it has to be ensured that the latter cover the required information.
The basic prerequisite is that objects can be distinguished and identified during tracing runs.
Since the Squeak environment does not offer an adequate functionality, the strategy that was applied to solve this issue is depicted in Section \ref{ss:ImplementationTracingIdentification}.
Since the tracing approach that served as basis of our \textsc{PathObjects} implementation does not cover object interactions, the extensions that are required therefore are described in Section \ref{ss:ImplementationTracing}.
