...
The size of the \inlinecode{involvedObjects} collection has a constant part of five words, but ultimately depends on the number of arguments of the according message.
...
\begin{equation}
s(o,m)=o*4 + m*5 + \sum_{n=1}^{m} (5 + a(M_n))\label{eq:SpaceConsumptionWords}
\end{equation}
...
Since a word consumes two bytes in the virtual machine we used, the total space consumption in bytes $s_B$ can be calculated with the help of formula \ref{eq:SpaceConsumptionBytes}. 
\begin{equation}
s_B(o,m) = 2 * s(o,m)\label{eq:SpaceConsumptionBytes}
\end{equation}
...
\subsubsection{Threats to Validity}
The calculated memory consumption should be seen as lower limit of the values that are reached in practice.
Although we took the overhead implicated by object instances in the Squeak virtual machine into consideration, the memory consumption will effectively be higher than determined by our calculations.
First, the memory overhead the virtual machine itself introduces to manage objects could not be considered with the chosen approach.
...
All those pitfalls of memory management can not be taken into consideration reasonably with our theoretical deduction.
