...
The applied algorithm has been designed with its interactive application in mind and generally performs fast enough therefor \cite{gansner_technique_1993}.
...
All results have been calculated by applying formula \ref{eq:SpaceConsumptionBytes} to the specific values of $o$, $m$, and $a(M_n)$ that have been measured per test case.
...
The test case \inlinecode{testWholeWordParsing} that covers a suitable scenario was provided as entry point.
...
Since the implementation is based on a \inlinecode{Dictionary}, a failing lookup of a search term is performed in constant time, while the lookup of actual synonyms is dependent on the number of found thesaurus entries.
...
First, participants had to identify the methods that are involved in the spell checking process, or, in other words, those that are called transitively starting from the \inlinecode{checkSpellingOf:} method.
...
In the case of groups B an C, each user study then started with a 15-minute introduction of the respective tools, since none of the participants had any prior experience with them.
...
The degree of comprehension is measured with the aid of a score-based grading of the answers participants gave, and expresses the percentage of points that were scored on a task (cf. Definition \ref{eq:UserEvaluationDc}).
...
\paragraph{Task 1} Strikingly, the mean time the participants in group B spent on this task is almost six minutes higher than that of group A.
...
This can be explained by the fact that many implementation details of the algorithm under investigation could be revealed by simply expanding the object state indicator of the covered camel case parser instance, and by stepping through the execution trace.
...
However, \inlinecode{MessageTally} is a sampling profiler, and the measurement of the execution of an unsuccessful search for synonyms was too fast to be captured by a sample.
...
Furthermore, the participants in group C require less time to gain this degree of comprehension in three out of five tasks.
The mean processing time of group A is lower in the case of tasks one and four, but this advantage comes at the cost of a distinctly reduced degree of comprehension.
The participants that were allowed to use \textsc{PathFinder} outperform group C only in the case of task three.
...
Since these tasks were derived from frequent developer questions, it can be concluded that our proposed information presentation actually can help in the comprehension of object-oriented behavior in general.

\begin{figure}[tb]
	\centering

	\includegraphics[width=1.0\textwidth]{../plots/05-UserEvaluation}
	\caption[Results of the User Study]{Performance of the participant groups (A: Squeak standard tools, B: \textsc{PathFinder}, C: \textsc{PathObjects}) per task, measured by processing time (lower is better) and degree of comprehension (higher is better); whiskers indicate the lowest and highest occurrences.}
	\label{fig:DiscussionStudyResults}
	
\end{figure}

\subsection{Threats to Validity}
Our experimental setup exhibits characteristics that could limit the validity of its outcomes:
...
Even though participants were selected on the basis of their relevant experiences and assigned to the different groups randomly, we cannot foreclose that differences in the mental abilities had an impact on the mean performance of the compared groups.
To rule out this factor, the user study would have to be conducted with a larger number of participants, which was not possible within the scope of this thesis.
Since the user study was supervised by the implementer of \textsc{PathObjects} and was not conducted double blinded, we also cannot foreclose that experimenter bias had an influence on the performance characteristics of the participants.
...
However, factors like ambient sounds, form on the day, fitness depending on the time of the day, the general wellbeing of participants, and others were beyond our control.
