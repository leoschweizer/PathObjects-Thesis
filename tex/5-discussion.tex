...
Consequently, participants could score one point at the maximum on this task.
...
Participants were asked to give duration estimations for searches that do and do not return results, whereby they also were allowed to make us eof profiling utilities.
The answers had to be selected from a given list of time values, and consequently the maximum possible score for this task were two points.
...
Participants could score a maximum of five points: one for the correct number, two for the locations where instances get created, and two for the reasons that lead to the creation of further instances.

\paragraph{Task 5} In this task, the participants were challenged to find the last changes of the spell checking process, which already served as object of investigation in Task 4.
...
Consequently, the maximum possible score for this tasks were three points.
There are two different problems with this task.
First, one has to identify the methods that are involved in the spell checking process, or, in other words, those that are called transitively starting from the \inlinecode{checkSpellingOf:} method.
...
\begin{figure}[tb]
	...
	\includegraphics[width=1.0\textwidth]{../plots/05-UserTask1}
	\caption[]{}
