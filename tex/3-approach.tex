...
In order to avoid the waiting time overhead of traditional dynamic analysis tools, we propose to utilize step-wise run-time analysis as underlying tracing mechanism.
Thus, the immediate provisioning of interactive object communication diagrams can be made possible.
...
For instance, the \inlinecode{AnalogClock class} object and the clock timer and analog clock instances are involved in the \inlinecode{attach:} message in Figure \ref{fig:ApproachBasicBefore}, namely as sender, receiver, and argument of the message.
...
both the part of the selector as well as the appropriate temporary variable name are displayed as flag alongside the object.
...
As can be seen from Listing \ref{lst:ApproachExample}, the \inlinecode{attach:} message does not have an explicit return statement.
...
For instance, Figure \ref{fig:ApproachBasicBefore} shows the internal state of the clock timer instance before the execution of the \inlinecode{attach:} message.
...
For instance, if one chooses to focus on the digital clock instance, the \inlinecode{SAnalogClock} class object and the instance of \inlinecode{SAnalogClock} will be removed from the canvas.
