...
Perscheid et al. have proposed Step-Wise Run-Time Analysis to deal with this problem, but the verification of applicability of this approach for the visualization of object interactions is still pending.

Third, well-known diagram notations that cover objects and their states and interactions like those included in the Unified Modeling Language are of static nature.
Mainly for this reason, they seem ill-suited for the use in conjunction with execution traces.
They grow fast, are not capable to cover the progress of object states over the course of time, and generally do not ease perceivability when used to visualize scenarios of larger scale, such as those resulting from unit test executions.
...
As depicted in Section \ref{missing}\todo{cite}, interactivity is an integral part of \textsc{PathObjects} diagrams.
