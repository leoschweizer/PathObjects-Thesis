...
Although the visualization of object interactions itself and the effectiveness of proposed notations and established standards like those specified by the Unified Modeling Language have not been a broad research topic so far, the requirement to clarify how object oriented systems actually behave at runtime has been well recognized in different research areas.

First and foremost, trace analysis tools from various areas like reverse engineering or automated system design recovery typically offer visualizations to aid in the understanding of the traces.
In this field, which is presented in Section \ref{s:RelatedTraceVis}, the predominant notation are diagrams similar or identical to sequence diagrams.
Another domain that deals with the problem of exposing object oriented program behavior is debugging.
...
Lastly, the field of education in computer sciences has similar objectives to first-mentioned research areas when teaching the concepts of object orientation at various stages in the curriculum.
...
The motives and techniques execution traces are generated with are manifold.
Reverse engineering, back-in-time debugging, and design and architecture reconstruction are popular areas of application.
...
Albeit being focused on activities during during debugging, all these approaches aim at easing the investigation of objects and their interactions and thus have similar objectives to the ones \textsc{PathObjects} pursues.
